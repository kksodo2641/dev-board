...
게시판 페이징 계산 로직은 BoardPagingUtils로 분리하였다.

현재는 페이징의 동작 원리를 직접 이해하고 구현하기 위해 Spring Data JPA의 `Page`와 `Pageable`을 사용하지 않고 자체 구현 방식을 적용하였다.

이를 통해 다음과 같은 기능을 직접 설계하고 구현하였다.
...
추후 Spring Data JPA의 `Page`와 `Pageable`을 활용하는 방향으로 리팩토링을 진행하여, 직접 구현한 방식과 프레임워크가 제공하는 방식의 차이점과 장단점을 비교·분석할 예정이다.
