...
## 4. 로그인 `redirectURL` 검증 누락으로 인한 오픈 리다이렉트 문제
...
Dev Board는 비로그인 사용자가 로그인이 필요한 화면에 접근할 경우,
로그인 성공 후 원래 화면으로 복귀할 수 있도록 `redirectURL`을 사용한다.

로그인 후 이동 흐름을 검토하던 중, 클라이언트가 전달한 `redirectURL`이
애플리케이션 내부 경로인지에 대한 검증 없이 리다이렉트 응답에 그대로 사용되고 있음을 확인했다.  
이 경우 공격자가 외부 사이트 주소를 포함한 로그인 URL을 전달하면,
사용자는 정상적인 Dev Board 로그인 화면에서 로그인 후 의도하지 않은 외부 사이트로 이동할 수 있다.

### 재현 환경 및 조건

로그아웃 상태에서 로그인 페이지의 `redirectURL`에 다음 값을 전달하고 로그인을 진행했다.

|      구분      |     `redirectURL`     |
|:------------:|:---------------------:|
|   정상 내부 경로   |       `/boards`       |
|  외부 절대 URL   | `https://example.com` |
| authority 형식 |    `//example.com`    |
| 다중 슬래시 우회 형식 |   `///example.com`    |

정상 내부 경로(예: `/boards`)를 전달한 경우에는 로그인 성공 후 해당 내부 경로로 이동했다.

그러나 외부 절대 URL과 `//` 또는 `///`로 시작하는 값을 전달한 경우에는
로그인 요청이 `302 Found`로 처리된 뒤 브라우저가 실제 외부 사이트로 이동했다.  
특히 `///example.com`은 응답의 `Location` 헤더에 `http:///example.com`으로 설정되었으며,
브라우저에서는 최종적으로 `https://example.com`으로 이동했다.

### 기대 동작

로그인 성공 후 이동 가능한 대상은 애플리케이션 내부 경로로 제한해야 한다.
...
외부 URL 또는 유효하지 않은 경로
→ 로그인 성공
→ 기본 경로(/)로 이동
```

잘못된 `redirectURL`이 전달된 경우, 로그인 자체를 실패시키지는 않고
인증 결과는 유지한 채 이동 경로만 안전한 기본값으로 대체한다.

### 원인 분석

`MemberController.login()`은 로그인 성공 후
클라이언트가 전달한 `redirectURL`을 별도의 검증 없이 리다이렉트 문자열에 연결하고 있었다.

```java
@PublicAccess
@PostMapping("/login")
public String login(final @RequestParam(defaultValue = "/") String redirectURL,
                    ...) {
    // ...
...
`redirectURL`은 로그인 페이지를 생성하는 서버나 클라이언트가 정상적인 내부 경로를 구성할 수도 있지만,
최종적으로는 HTTP 요청 파라미터이므로 사용자가 임의로 변경할 수 있다.  
따라서 항상 서버 내부에서 생성된 값이라는 가정만으로 신뢰할 수 없으며,
리다이렉트에 사용하기 전에 최종 목적지를 검증해야 한다.

외부 절대 URL뿐만 아니라 `//example.com`처럼 scheme 없이 authority를 나타내는 값도
브라우저가 외부 사이트 주소로 해석할 수 있다.  
`///example.com` 역시 Spring의 리다이렉트 처리와 브라우저의 URL 해석을 거치면서
외부 사이트로 이동할 수 있다.

Dev Board에서는 이처럼 검증되지 않은 `redirectURL`을 그대로 사용하여,
로그인 후 외부 사이트로 이동할 수 있는 오픈 리다이렉트(open redirect) 문제가 발생했다.

### 해결 과정 및 설계 결정

로그인 성공 후 이동 경로를 애플리케이션 내부의 절대 경로 형식으로 제한했다.

`java.net.URI`로 `redirectURL`을 파싱하고 다음 조건을 모두 만족하는 경우에만
원본 값을 리다이렉트에 사용한다.

- 올바른 URI 형식
- 명시적인 scheme과 authority 없음
- 원본 값과 해석된 path가 단일 슬래시(`/`)로 시작
- 이중 슬래시(`//`)로 시작하거나 역슬래시(`\`)를 포함하지 않음
- fragment 없음

단, 쿼리 문자열은 이동할 애플리케이션 경로를 변경하지 않으므로 별도로 제한하지 않는다.  
즉, `/boards?page=2`와 `/boards?keyword=https://example.com` 같은 내부 경로는 그대로 유지된다.

외부 URL이나 유효하지 않은 경로가 전달되면 기본 경로인 `/`로 대체한다.

```java
@PublicAccess
@PostMapping("/login")
public String login(final @RequestParam(defaultValue = DEFAULT_REDIRECT_URL) String redirectURL,
                    ...) {
    // ...
...
URI 파싱 결과만으로 판단하기 어려운 우회 형식도 있으므로
원본 문자열과 파싱된 경로를 함께 검증한다.

예를 들어, `///example.com`은 URI 파싱 결과에서 authority가 `null`이고,
path가 `/example.com`으로 해석될 수 있으므로 원본 문자열의 시작 형식도 확인한다.  
반대로 `/%2Fexample.com`과 같은 인코딩된 값은 디코딩된 path를 함께 확인하여 차단한다.

현재 `redirectURL`은 로그인 성공 응답에서만 최종적으로 사용되므로,
검증 책임은 `MemberController`에 두었다.  
`MemberService`는 회원 인증에 대한 책임만 담당하고, HTTP 리다이렉트 정책을 알지 않는 기존 방식을 유지한다.

### 최종 해결 내용

로그인 성공 후 이동 경로를 다음 정책으로 처리하도록 변경했다.

|         요청한 이동 경로          |         처리 결과         |
|:--------------------------:|:---------------------:|
|         `/boards`          |     요청한 내부 경로로 이동     |
|      `/boards?page=2`      | 쿼리 문자열을 포함한 내부 경로로 이동 |
|   `https://example.com`    |     기본 경로 `/`로 이동     |
|      `//example.com`       |     기본 경로 `/`로 이동     |
|      `///example.com`      |     기본 경로 `/`로 이동     |
|   인코딩 또는 역슬래시를 이용한 우회 형식   |     기본 경로 `/`로 이동     |
| 상대 경로, fragment 또는 잘못된 URI |     기본 경로 `/`로 이동     |

유효하지 않은 이동 경로가 전달되더라도 로그인 세션은 정상적으로 생성되며, 애플리케이션 홈 화면으로 이동한다.

### 테스트 및 재검증

`MemberControllerTest` 통합 테스트를 추가하여 다음 동작을 검증했다.

- `redirectURL` 부재 시 기본 경로(`/`)로 이동하는지 확인
- 정상적인 내부 경로와 쿼리 문자열이 유지되는지 확인
- 외부 절대 URL과 authority 형식이 기본 경로로 대체되는지 확인
- 상대 경로, 다중 슬래시, 인코딩 및 역슬래시 우회 형식이 차단되는지 확인
- fragment와 잘못된 URI 형식이 차단되는지 확인
- 유효하지 않은 이동 경로가 전달되는 경우에도 로그인 세션이 정상적으로 생성되는지 확인

수정 전과 동일한 경로를 사용하여 브라우저에서 재검증을 수행했다.  
정상 내부 경로는 기존과 같이 유지됐으며,
외부 절대 URL과 `//` 또는 `///`로 시작하는 경로는
더 이상 외부 사이트로 연결되지 않고 모두 기본 홈 화면으로 이동했다.

### 회고

사용자가 입력한 리다이렉트 경로는 정상적인 로그인 화면을 거치더라도
서버가 신뢰할 수 있는 값이 아님을 확인했다.  
특히 URL은 표현 방식에 따라 동일하거나 유사한 외부 목적지를 나타낼 수 있으므로,
단순 문자열 비교보다는 원본 값과 URI 파싱 결과를 함께 확인해야 한다.

이번 수정에서는 로그인 후 이동 가능한 대상을 애플리케이션 내부 경로로 제한하고,
유효하지 않은 경로는 로그인 결과를 유지한 채 안전한 기본 경로로 대체했다.  
또한 정상 경로와 외부 URL 및 우회 형식을 Controller 통합 테스트와
브라우저 재검증으로 확인하여, 동일한 오픈 리다이렉트 문제가 다시 발생하지 않도록 했다.
