...
Controller는 다음 역할을 수행한다.

- 요청 수신 및 요청 데이터 검증
- Service 호출
- 화면 요청의 Model 구성 및 View 반환
- API 요청의 HTTP 응답 구성
...
게시글 조회수는 게시글의 관심도와 활동량을 나타내는 지표이다.

그러나 단순히 상세 조회 요청이 발생할 때마다 조회수를 증가시키면,
새로고침이나 반복 접근만으로 조회수가 쉽게 증가할 수 있다.  
또한 회원과 비회원, 작성자와 관리자에게 서로 다른 정책을 적용할지와
삭제된 게시글을 조회수 증가 대상으로 볼지도 결정해야 했다.

따라서 어떤 요청을 게시글 조회로 인정하고,
동일한 사용 환경의 반복 조회를 어느 범위까지 허용할지에 대한 정책이 필요했다.

### 고려한 대안

- **게시글 상세 조회 요청마다 조회수 증가**
  - 구현은 단순하지만, 새로고침과 반복 접근으로 조회수가 쉽게 증가할 수 있다.

- **일정 기간 동안 동일 게시글의 중복 증가 제한**
  - 실제 사용자 수를 정확하게 측정하지는 못하지만,
    동일한 사용 환경의 반복 조회로 인한 조회수 왜곡을 줄일 수 있다.

- **작성자와 관리자의 조회 제외**
  - 운영 목적의 접근을 조회수 증가 대상에서 제외할 수 있지만,
    사용자 역할에 따라 조회 정책이 달라지고 추가적인 권한 판별이 필요하다.

- **회원 유형과 관계없이 동일한 정책 적용**
  - 작성자와 관리자를 포함한 모든 사용자의 상세 조회를 동일하게 처리할 수 있다.
  - 운영 목적의 조회도 조회수에 포함된다.

### 결정

**`ACTIVE` 게시글의 상세 조회 요청을 조회로 인정하고,
동일한 사용 환경에서는 24시간 동안 동일 게시글의 조회수를 한 번만 증가시킨다.**

#### 조회수 증가 대상

회원과 비회원 모두 동일한 정책을 적용한다.  
게시글 작성자와 관리자도 별도로 제외하지 않고 조회수 증가 대상에 포함한다.
...
동일한 사용 환경에서 이미 조회한 게시글은 24시간 동안 조회수를 다시 증가시키지 않는다.  
중복 여부를 판단하는 구체적인 방법은 `AD-023. Cookie 기반 조회수 중복 증가 방지`를 따른다.
...
- 반복 새로고침이나 재접속에 의한 조회수 왜곡을 줄일 수 있다.
- 회원 여부나 역할에 따라 별도의 조회수 정책을 적용하지 않아도 된다.
- 작성자와 관리자도 게시글을 실제로 확인하는 사용자이므로,
  일반 사용자와 동일하게 처리하는 것이 정책의 일관성과 구현의 단순성에 적합하다.
- 삭제된 게시글은 일반적인 상세 조회 대상이 아니므로 조회수에 포함하지 않는 것이 자연스럽다.
- 24시간은 단기간의 반복 접근을 제한하면서도
  이후의 재방문은 새로운 조회로 인정할 수 있는 기간이라고 판단했다.

### 결과 및 트레이드오프

- `ACTIVE` 게시글의 상세 조회에 일관된 조회수 증가 정책 적용
- 동일한 사용 환경의 반복 조회로 인한 조회수 왜곡 완화
- 회원·비회원, 작성자·관리자에게 동일한 정책 적용
- 존재하지 않거나 삭제된 게시글의 조회수 증가 방지
- 조회수는 게시글의 상대적인 관심도를 나타내는 지표일 뿐,
  고유 사용자 수나 실제 열람 횟수를 정확하게 측정하는 데 한계 존재
...
`AD-022. 게시글 조회수 증가 정책`에 따라,
동일한 사용 환경에서는 24시간 동안 동일 게시글의 조회수를 한 번만 증가시켜야 한다.  
이를 위해 회원과 비회원 모두에게 적용할 수 있는 조회 이력 저장 방식이 필요했다.

특히 여러 게시글의 조회 이력을 하나의 저장 단위에서 함께 관리할지,
게시글별로 분리하여 관리할지 결정해야 했다.  
또한 선택한 저장 방식에 맞춰 조회 이력 처리와
조회수 증가 비즈니스 로직의 계층별 책임도 함께 결정해야 했다.
...
- **Cookie 기반 관리**
  - 별도의 서버 저장소 없이 회원과 비회원에게 동일하게 적용할 수 있다.
  - 사용자가 Cookie를 삭제하거나 다른 브라우저를 사용하면 조회 이력이 공유되지 않는다.

- **Session 기반 관리**
  - 서버에서 사용자별 조회 이력을 관리할 수 있다.
  - 서버가 상태를 유지해야 하며, Session 만료 이후에는 조회 이력이 유지되지 않는다.

- **DB 또는 Redis 기반 관리**
  - 서버에서 더 정확하고 지속적인 조회 이력을 관리할 수 있다.
  - 비회원 식별 정책과 별도의 저장 구조가 필요하며,
    현재 프로젝트 규모에 비해 구현 및 운영 복잡도가 크다.
...
- **게시글마다 별도의 Cookie 생성**
  - 게시글별 조회 여부를 단순하게 확인할 수 있다.
  - 조회한 게시글이 늘어날수록 Cookie 수도 함께 증가한다.

- **하나의 Cookie에 여러 게시글 ID 저장**
  - Cookie 수를 제한하면서 여러 게시글의 조회 여부를 관리할 수 있다.
  - 조회한 게시글 ID가 누적되므로 Cookie 크기 제한을 별도로 고려해야 한다.

---

#### Cookie 처리 계층

- **Service에서 조회수 증가와 Cookie를 함께 처리**
  - 하나의 메서드에서 조회수 증가와 조회 이력 기록을 함께 처리할 수 있다.
  - 비즈니스 계층이 HTTP 요청 및 응답 정보에 의존하게 된다.

- **Controller에서 Cookie 처리 / Service에서 조회수 증가**
  - Controller는 HTTP 요청과 응답의 Cookie를 처리한다.
  - Service는 `ACTIVE` 게시글 검증과 조회수 증가만 담당한다.
  - 계층별 책임은 분리되지만, Controller가 두 작업의 실행 순서를 조정해야 한다.

### 결정

**조회 이력은 하나의 Cookie에서 관리하고, Controller와 Service의 책임을 분리한다.  
새로운 조회 대상은 조회수 증가가 성공한 경우에만 Cookie에 기록한다.**

#### Cookie 기반 조회 이력 관리

회원과 비회원 모두에게 동일한 중복 조회 정책을 적용하기 위해 브라우저 Cookie에 조회한 게시글 ID를 저장한다.  
Cookie의 유효기간은 24시간으로 설정하고, 해당 Cookie가 게시판 기능에만 전달되도록 경로를 `/boards`로 제한한다.

#### 하나의 Cookie에 여러 게시글 ID 저장

`viewedBoards` Cookie 하나에 조회한 게시글 ID를 구분자로 연결하여 저장한다.

```text
viewedBoards=15:21:35
```

요청 Cookie에 현재 게시글 ID가 이미 존재하면,
조회수를 증가시키지 않으며 새로운 Cookie를 응답에 추가하지 않는다.

#### Controller와 Service의 책임 분리

Controller는 요청 Cookie를 읽어 중복 조회 여부를 판단하고,
응답에 조회 기록 Cookie를 추가하는 HTTP 처리를 담당한다.  
Service는 `ACTIVE` 게시글을 검증하고, 게시글의 조회수를 증가시키는 비즈니스 로직만 담당한다.

Cookie 기록 여부는 Service의 조회수 증가 성공을 기준으로 판단한다.  
Cookie에 기록되지 않은 새로운 조회 대상이면
Controller는 Service의 조회수 증가를 먼저 요청하고,
성공한 경우에만 새로운 게시글 ID를 Cookie에 기록한다.

```text
Cookie 조회 및 중복 여부 판단
→ 새로운 Cookie 값 계산
→ Service에서 ACTIVE 게시글 조회 및 조회수 증가
→ Cookie 객체 생성 및 응답 추가
```

존재하지 않거나 삭제된 게시글은 조회수 증가 과정에서 예외가 발생하므로,
조회 기록 Cookie를 응답에 추가하지 않는다.

다만, 이는 이후의 상세 조회, 권한 확인 및 View 렌더링까지 하나의 원자적인 작업으로 처리한다는 의미는 아니다.

### 이유

- 별도의 서버 저장소 없이 회원과 비회원에게 동일한 중복 조회 정책을 적용할 수 있다.
- 하나의 Cookie를 사용하여 게시글마다 Cookie가 생성되는 것을 방지할 수 있다.
- HTTP 정보인 Cookie는 Controller가 처리하고,
  Service는 게시글 검증 및 조회수 증가에 집중할 수 있다.
- 조회수 증가에 실패한 요청을 조회 이력에 기록하지 않음으로써,
  조회수 증가 결과와 Cookie 상태의 일관성을 유지할 수 있다.
- 현재 프로젝트 규모에서는 DB나 Redis 기반의 정밀한 조회 이력보다
  Cookie 기반의 단순한 방식이 더 적절하다고 판단했다.

### 결과 및 트레이드오프

- 회원과 비회원 모두에게 동일한 조회수 중복 증가 방지 정책 적용
- 서버에 별도의 조회 이력을 저장하지 않는 단순한 구조 유지
- HTTP 처리와 비즈니스 로직의 책임 분리
- 조회수 증가에 성공한 게시글만 조회 기록 Cookie에 반영
- 존재하지 않거나 삭제된 게시글의 잘못된 조회 기록 방지
- 사용자의 Cookie 삭제, 브라우저 변경 또는 시크릿 모드 사용 시 중복 조회 제한 우회 가능
- 조회한 게시글 ID가 하나의 Cookie에 누적되므로, 브라우저의 Cookie 크기 제한을 별도로 검토해야 함
...
- Comment 구현으로 삭제 게시글과 연관 데이터의 조회·표시 정책 검토 필요
- BoardLike 등 추가 연관 도메인 도입 예정
- 현재 Soft Delete 정책을 적용하고 있어 정책 확장 가능
